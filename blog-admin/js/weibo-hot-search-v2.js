...
class HotSearchManager {
    constructor() {
        ...
        this.scrollSpeed = 0.3; // 滚动速度（像素/帧）
    ...
    async fetchDouyinHot() {
        try {
            const response = await this.fetchWithTimeout('https://tenapi.cn/v2/douyinhot');
    ...
    async fetchWeiboHot() {
        try {
            const response = await this.fetchWithTimeout('https://tenapi.cn/v2/weibohot');
            const result = await response.json();
            console.log('微博热搜数据:', result);
            if (result.code === 200 && result.data && Array.isArray(result.data)) {
                return result.data.slice(0, 20).map(item => ({
                    title: item.title || item.word || '未知',
    ...
    async fetchTencentHot() {
        try {
            const response = await this.fetchWithTimeout('https://tenapi.cn/v2/tophub');
            const result = await response.json();
            console.log('腾讯热搜数据:', result);
            if (result.code === 200 && result.data && Array.isArray(result.data)) {
                return result.data.slice(0, 20).map(item => ({
                    title: item.title || item.word || '未知',
                    url: item.url || item.link || 'https://new.qq.com/',
                    hot: item.hot_value || item.hot || 0
                }));
            }
        } catch (error) {
            console.error('腾讯热搜API失败:', error);
        }
        return null;
    }
    ...
    async fetchBaiduHot() {
        try {
            const response = await this.fetchWithTimeout('https://tenapi.cn/v2/baiduhot');
            const result = await response.json();
            console.log('百度热搜数据:', result);
            if (result.code === 200 && result.data && Array.isArray(result.data)) {
                return result.data.slice(0, 20).map(item => ({
                    title: item.title || item.word || '未知',
    ...
    async fetchGlobalHot() {
        try {
            const response = await this.fetchWithTimeout('https://tenapi.cn/v2/zhihuhot');
            const result = await response.json();
            console.log('全球热搜数据:', result);
            if (result.code === 200 && result.data && Array.isArray(result.data)) {
                return result.data.slice(0, 20).map(item => ({
                    title: item.title || item.word || '未知',
                    url: item.url || item.link || 'https://www.zhihu.com/',
                    hot: item.hot_value || item.hot || 0
                }));
            }
        } catch (error) {
            console.error('全球热搜API失败:', error);
        }
        return null;
    }
