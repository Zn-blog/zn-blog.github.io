...
class ArticleEditor {
    ...
    async init() {
        ...
        console.log('🔍 编辑器环境检测:', {
            ...
            useApi: window.blogDataStore.useApi
        });
        
        if (isVercelEnv) {
            console.log('🚫 Vercel环境：强制禁用JSON文件加载，启用API模式');
            // 强制设置为不使用JSON文件
            window.blogDataStore.useJSONFiles = false;
            window.blogDataStore.useApi = true;
        ...
        // 确保API基础URL正确设置
        const apiBase = window.blogDataStore.getApiBaseURL();
