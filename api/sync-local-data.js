...
export default async function handler(req, res) {
  ...
  try {
    console.log('🔄 开始同步本地JSON数据到KV数据库...');
    
    // 检查是否已经同步过
    const syncStatus = await kv.get('local_sync_status');
    ...
    // 逐个保存数据到KV
    for (const [key, data] of Object.entries(dataStructure)) {
      try {
        await kv.set(key, data);
        const recordCount = Array.isArray(data) ? data.length : 1;
        totalRecords += recordCount;
        results[key] = { status: 'success', records: recordCount };
        console.log(`✅ 同步完成: ${key} (${recordCount}条记录)`);
      } catch (error) {
    ...
    // 标记同步完成
    await kv.set('local_sync_status', 'completed');
    await kv.set('local_sync_date', new Date().toISOString());
    await kv.set('local_sync_results', results);

    console.log('🎉 本地数据同步完成!');

    return res.json({
      success: true,
      message: `本地数据同步完成，共同步 ${totalRecords} 条记录`,
      totalRecords,
      results,
    ...
    return res.status(500).json({
      success: false,
      message: '本地数据同步失败: ' + error.message
    });
  }
}